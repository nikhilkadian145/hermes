...
```
1. db.getOutstandingInvoices()            → all UNPAID + PARTIAL invoices
2. db.groupByClient(invoices)             → aggregate per client
3. [optional] pdf.generateOutstandingReport(data)
```
...
```
pdf.generateOutstandingReport(data) → data/reports/Outstanding_15Jan2025.pdf
```
