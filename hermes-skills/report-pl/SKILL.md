...
```
1. db.getRevenueByPeriod({from, to})        → paid invoices (income)
2. db.getExpensesByPeriod({from, to})       → logged expenses by category
3. pdf.generatePLReport({income, expenses, period})  → PDF
```
...
```
pdf.generatePLReport(data) → data/reports/PL_Jan2025.pdf
telegram.sendDocument(chatId, pdfPath)
```
