...
```
1. db.getClient(clientName)          → fetch or prompt to create client
2. db.getNextInvoiceNumber()         → auto-increment invoice number
3. db.createInvoice({...})           → persist invoice + line items to SQLite
4. pdf.generateInvoice(invoiceId)    → render PDF via Puppeteer template
