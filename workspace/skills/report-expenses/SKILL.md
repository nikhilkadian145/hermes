...
```
1. DbListExpensesTool({from, to})          → all expenses in range
2. db.groupExpensesByCategory(expenses)         → category totals
3. [optional] PdfGenerateExpenseReportTool(data)   → full PDF
```
...
```
PdfGenerateExpenseReportTool(data) → data/reports/Expenses_Jan2025.pdf
```
