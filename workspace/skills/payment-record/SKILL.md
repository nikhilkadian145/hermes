...
```
1. DbListInvoicesTool(clientName, amount)    → match invoice
2. DbRecordPaymentTool({invoiceId, amount, mode, date, utr})
3. DbUpdateInvoiceStatusTool(invoiceId)           → PAID if full, PARTIAL if not
